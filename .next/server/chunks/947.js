...
/* harmony export */   "AverageColor": () => (/* binding */ AverageColor),
/* harmony export */   "ContrastColor": () => (/* binding */ ContrastColor),
/* harmony export */   "DeltaEColor": () => (/* binding */ DeltaEColor),
/* harmony export */   "DistanceColor": () => (/* binding */ DistanceColor),
/* harmony export */   "GeneratePalette": () => (/* binding */ GeneratePalette),
/* harmony export */   "LuminanceColor": () => (/* binding */ LuminanceColor),
/* harmony export */   "MaxColor": () => (/* binding */ MaxColor),
/* harmony export */   "MedianColor": () => (/* binding */ MedianColor),
/* harmony export */   "MinColor": () => (/* binding */ MinColor),
/* harmony export */   "MixColor": () => (/* binding */ MixColor),
/* harmony export */   "P3toCMYK": () => (/* binding */ P3toCMYK),
/* harmony export */   "P3toSRGB": () => (/* binding */ P3toSRGB),
/* harmony export */   "TemperatureColor": () => (/* binding */ TemperatureColor)
/* harmony export */ });
/* harmony import */ var chroma_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(6303);
...
const GeneratePalette = (props)=>{
    ...
    if (!startColor || !endColor) {
        ...
        while(!colorNum){
            chroma_js__WEBPACK_IMPORTED_MODULE_0___default().scale([
                startColor,
                endColor
            ]).mode("hsl").colors(5).forEach((color)=>{
        ...
        while(colorNum){
            chroma_js__WEBPACK_IMPORTED_MODULE_0___default().scale([
                startColor,
                endColor
            ]).mode("hsl").colors(colorNum).forEach((color)=>{
                colorPalette.push(color);
            });
            return colorPalette;
        }
...
const MixColor = (props)=>{
    ...
    if (!colorA || !colorB) {
        ...
        while(!mixRatio){
            const mixColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().mix(colorA, colorB, 0.5, colorModel).hex();
            return mixColor;
        }
        //如果没有传入colorModel, 则默认色彩模式为hsl
        while(!colorModel){
            const mixColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().mix(colorA, colorB, mixRatio, "hsl").hex();
            return mixColor;
        }
        //如果没有传入mixRatio, colorModel
        while(!mixRatio && !colorModel){
            const mixColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().mix(colorA, colorB, 0.5, "hsl").hex();
            return mixColor;
        }
        //如果传入了mixRatio, colorModel
        while(mixRatio && colorModel){
            const mixColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().mix(colorA, colorB, mixRatio, colorModel).hex();
            return mixColor;
        }
    }
...
const AverageColor = (props)=>{
    ...
    if (!colors) {
        ...
        const averageColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().average(colors, "hsl", [
            1,
            1
        ]).hex();
        return averageColor;
        ...
        while(!colorModel){
            const averageColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().average(colors, "hsl", colorWeights).hex();
            return averageColor;
        }
        ...
        while(!colorWeights){
            ...
            // console.log(weights) 
            const averageColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().average(colors, colorModel, weights).hex();
            return averageColor;
        }
        ...
        while(!colorModel && !colorWeights){
            ...
            const averageColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().average(colors, "hsl", weights).hex();
            return averageColor;
        }
        //如果传入了colorModel, colorWeights
        while(colorModel && colorWeights){
            const averageColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().average(colors, colorModel, colorWeights).hex();
            return averageColor;
        }
    }
...
const ContrastColor = (props)=>{
    ...
    if (!colorA || !colorB) {
        ...
        //获取对比度
        const contrastColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().contrast(colorA, colorB);
        return contrastColor;
    }
// console.log(contrastColor)   
};
//05.计算两种颜色之间的欧氏距离：chroma.distance(color1, color2, mode='lab') (欧氏距离越大，两种颜色越不相似)
const DistanceColor = (props)=>{
    ...
    if (!colorA || !colorB) {
        ...
        //如果没有传入colorModel, 则默认色彩模式为hsl
        while(!colorModel){
            const distanceColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().distance(colorA, colorB, "hsl");
            return distanceColor;
        }
        //如果传入了colorModel
        while(colorModel){
            const distanceColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().distance(colorA, colorB, colorModel);
            return distanceColor;
        }
    }
...
const DeltaEColor = (props)=>{
    ...
    if (!colorA || !colorB) {
        ...
        //获取deltaE值
        const deltaEColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default().deltaE(colorA, colorB);
        return deltaEColor;
    }
// console.log(deltaEColor)   
};
//07.更改颜色的luminance值：color.luminance([lum, mode='rgb']) (luminance值越大，颜色越亮，luminance值越小，颜色越暗)
const LuminanceColor = (props)=>{
    ...
    if (!color) {
        ...
        //获取更改后的颜色
        const luminanceColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default()(color).luminance(luminance).hex();
        return luminanceColor;
    }
// console.log(luminanceColor)
};
//08.获取颜色的temperature值 (temperature值越大，颜色越暖，temperature值越小，颜色越冷)
const TemperatureColor = (props)=>{
    //color: 颜色
    const { color  } = props;
    //如果没有传入color, 则返回null
    if (!color) {
        ...
        //获取temperature值
        const temperatureColor = chroma_js__WEBPACK_IMPORTED_MODULE_0___default()(color).temperature();
        return temperatureColor;
    }
// console.log(temperatureColor)   
};
//09.P3色域转SRGB色域
const P3toSRGB = (props)=>{
    //color: 颜色的hex值
    const { color  } = props;
    //如果没有传入color, 则返回null
    if (!color) {
    ...
    //将hex转换为RGB
    const [r, g, b] = chroma_js__WEBPACK_IMPORTED_MODULE_0___default()(color).rgb();
    //将RGB转换为线性RGB
    const r1 = r / 255;
    const g1 = g / 255;
    const b1 = b / 255;
...
const P3toCMYK = (props)=>{
    ...
    //将P3颜色转换为sRGB颜色
    const colorSRGB = P3toSRGB({
        color: color
    });
    //将sRGB颜色转换为CMYK颜色
    const CMYK = chroma_js__WEBPACK_IMPORTED_MODULE_0___default()(colorSRGB).cmyk();
    //将CMYK去除小数点后面的值
    // CMYK.forEach((color,index) => {
    //     CMYK[index] = Math.round(color * 100)
    // })
    //将CMYK颜色转换为hex颜色
    const colorCMYK = chroma_js__WEBPACK_IMPORTED_MODULE_0___default()(CMYK).hex();
    // console.log(colorCMYK)
    return colorCMYK;
};
//11.求多个颜色值中的中位数
const MedianColor = (props)=>{
    //colors: 颜色数组
    const { colors  } = props;
