...
import { WeatherBg, WeatherIcon, WeatherCard } from "../components/weatherParse";
import { 
    ChartWindBg, 
    ChartWindPointerB, 
    IconBody, 
    ...
    IconRain, 
    IconUv, 
    IconWind } from "../components/icons";
...
export default function WeatherAPP() {
    ...
    return (
        <div className="relative w-[67.5rem] h-[24rem] rounded-2xl grid grid-cols-2 gap-3 p-8">
            ...
            {/* 动态天气背景 */}
            {/* <div className={`absolute z-0 top-0 left-0 w-[67.5rem] h-[24rem] rounded-2xl ${WeatherBg({cloud:weatherData.now.cloud})}`}></div> */}
            <WeatherBgColor cloud={weatherData.now.cloud} />
        </div>
...
const WeatherTemp = (props) => {
    ...
    return(
        <div className="w-full h-full flex flex-col gap-6 z-20">
            <div className="w-full flex flex-row items-end gap-8 px-4">
                <div className="flex flex-row gap-1 items-center">
                    <p className="GCHheading-xl text-center">{temp}°</p> 
                    <div className="flex flex-col -mt-2 ">
                        <WeatherIcon iconId = {icon} iconSize = "56" />
                        <p className="GCHheading-sm text-center">{text}</p>
                    </div>  
                </div>
                <div className="flex flex-col mb-8">
                    <div className="flex flex-row justify-end">
                        <p className="GCHheading-sm text-end">{month}月</p>
                        <p className="GCHheading-sm text-end">{day}日</p>
                    </div>
                    <p className="GCHtext-xs text-end text-white/60">观测时间</p>
                    <p className="GCHtext-xs text-end text-white/60">{obsTime}</p>
                </div>   
            </div>

            <div className="w-full h-full grid grid-cols-[1fr_0.48fr] gap-3">
                <WeatherWind
                    wind360={wind360}
                    windScale={windScale}
                    windDir={windDir}
                    ...
                    cloud={cloud}
                />
                <WeatherPressure pressure={pressure} cloud={cloud} />
            </div>
...
const WeatherWind = (props) => {
    ...
    let windFell = '叶动红旗展'
    let wind = '微风'
    if( windScale == 0){windFell = '烟柱直冲天',wind = '无风'}
    if( windScale == 1){windFell = '青烟随风偏',wind = '软风'}
    if( windScale == 2){windFell = '风来吹脸面',wind = '轻风'}
    if( windScale == 3){windFell = '叶动红旗展',wind = '微风'}
    if( windScale == 4){windFell = '风吹纸片飞',wind = '和风'}
    if( windScale == 5){windFell = '带叶小树摇',wind = '劲风'}
    if( windScale == 6){windFell = '举伞步行艰',wind = '强风'}
    if( windScale == 7){windFell = '迎风走不便',wind = '疾风'}
    if( windScale == 8){windFell = '风吹树枝断',wind = '大风'}
    if( windScale == 9){windFell = '屋顶飞瓦片',wind = '烈风'}
    if( windScale == 10){windFell = '拔树又倒屋',wind = '狂风'}
    if( windScale > 10){windFell = '陆上很少见',wind = '台风'}

    return(
        <div className={`w-full h-full grid grid-cols-[1fr_0.5fr] gap-3 px-4 py-3 rounded-2xl ${WeatherCard({cloud:cloud})}`} >
            <div className=" flex flex-col justify-center">
                <div className="flex flex-row gap-2 items-center">
                    <IconWind w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                    <p className="GCHtext-xs text-white/60">风</p>
                </div>
                <div className=" flex flex-row gap-3 border-b border-white/10">
                    <p className="GCHheading-xs mt-[17px]">{windDir}</p>
                    <div className="flex flex-row gap-1 items-center">
                        <p className="GCHheading-md">{windScale}</p>
                        <div className="flex flex-col">
                            <p className="GCHtext-xs text-white/60">级</p>
                            <p className="GCHtext-xs text-white">{windFell}</p>
                        </div>
                    </div>
                </div>
                
                <div className="flex flex-row gap-1 items-center">
                    <p className="GCHheading-md">{windSpeed}</p>
                    <div className="flex flex-col">
                        <p className="GCHtext-xs text-white/60">公里/小时</p>
                        <p className="GCHtext-xs text-white">{wind}</p>
                    </div>
                </div>
            ...
            <div className="relative flex justify-center items-center">
                ...
                >
                    <ChartWindPointerB w = "100" h = "100" fill = "#ffffff" />
                </motion.div>
                <div className="absolute rounded-full"><ChartWindBg w = "100" h = "100"/></div>  
            </div>
        </div>
    )
}
...
const WeatherPressure = (props) => {
    ...
    return(
        <div className={`w-full h-full flex flex-col px-4 py-3 rounded-2xl ${WeatherCard({cloud:cloud})}`}>
            <div className="flex flex-row gap-2 items-center">
                <IconPressure w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                <p className="GCHtext-xs text-white/60">气压</p>
            </div>

            <div className="relative w-full flex items-center justify-center mt-9">
                <motion.div 
...
const WeatherFeelsLike = (props) => {
    ...
    if(30 < feelsLike && feelsLike <= 50){feel = '感觉炎热，小心中暑'}
    if(26 < feelsLike && feelsLike <= 30){feel = '温度较高，感觉炎热'}
    if(18 < feelsLike && feelsLike <= 26){feel = '温度舒适'}
    if(12 < feelsLike && feelsLike <= 18){feel = '温度较低，感觉凉爽'}
    if(6 < feelsLike && feelsLike <= 12){feel = '降温啦，注意保暖'}
    if(-6 < feelsLike && feelsLike <= 6){feel = '感觉寒冷，注意保暖'}
    if(-30 < feelsLike &&  feelsLike <= -6){feel = '温度过低，小心冻伤'}

    return(
        <div className={`w-full h-full z-20 px-4 py-3 flex flex-col justify-between rounded-2xl ${WeatherCard({cloud:cloud})}`}>
            <div className="w-full flex flex-col gap-2">
                <div className="flex flex-row gap-2 items-center">
                    <IconBody w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                    <p className="GCHtext-xs text-white/60">体感温度</p>
                </div>
                <p className="GCHheading-md">{feelsLike}°</p>
            </div>
            <p className="GCHtext-xs text-white">{feel}</p>
        </div>
    )
}
...
const WeatherRain = (props) => {
    ...
    return(
        <div className={`w-full h-full z-20 px-4 py-3 flex flex-col justify-between rounded-2xl ${WeatherCard({cloud:cloud})}`}>
            <div className="w-full flex flex-col gap-2">
                <div className="flex flex-row gap-2 items-center">
                    <IconRain w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                    <p className="GCHtext-xs text-white/60">降水</p>
                </div>
                <p className="GCHheading-md">{precip}<label className="GCHtitle-base">毫米</label></p>
            </div>
            <p className="GCHtext-xs text-white">当前小时累计降水量</p>
        </div>
    )
}
...
const WeatherHumidity = (props) => {
    ...
    if(0 < humidity && humidity <= 40){hum = '环境干燥'}
    if(40 < humidity && humidity <= 60){hum = '环境舒适'}
    if(60 < humidity && humidity <= 100){hum = '环境湿润'}
   
    return(
        <div className={`w-full h-full z-20 px-4 py-3 flex flex-col justify-between rounded-2xl ${WeatherCard({cloud:cloud})}`}>
            <div className="w-full flex flex-col gap-2">
                <div className="flex flex-row gap-2 items-center">
                    <IconHumidity w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                    <p className="GCHtext-xs text-white/60">湿度</p>
                </div>
                <p className="GCHheading-md">{humidity}<label className="GCHtitle-base">%</label></p>
            </div>
            <p className="GCHtext-xs text-white">{hum}</p>
        </div>
    )
}
...
const WeatherVisibility = (props) => {
    ...
    if(20 < vis ){feelVis = '视野清晰'}
    if(15 < vis && vis <= 20){feelVis = '视野较好'}
    if( 5 < vis && vis <= 15){feelVis = '视野一般'}
    if( vis <= 5){feelVis = '视野很差'}
   
    return(
        <div className={`w-full h-full z-20 px-4 py-3 flex flex-col justify-between rounded-2xl ${WeatherCard({cloud:cloud})}`}>
            <div className="w-full flex flex-col gap-2">
                <div className="flex flex-row gap-2 items-center">
                    <IconEye w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                    <p className="GCHtext-xs text-white/60">能见度</p>
                </div>
                <p className="GCHheading-md">{vis}<label className="GCHtitle-base">公里</label></p>
            </div>
            <p className="GCHtext-xs text-white">{feelVis}</p>
        </div>
    )
}
...
const WeatherDewPoint = (props) => {
    ...
    return(
        <div className={`w-full h-full z-20 px-4 py-3 flex flex-col justify-between rounded-2xl ${WeatherCard({cloud:cloud})}`}>
            <div className="w-full flex flex-col gap-2">
                <div className="flex flex-row gap-2 items-center">
                    <IconDew w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                    <p className="GCHtext-xs text-white/60">露点温度</p>
                </div>
                <p className="GCHheading-md">{dew}°</p>
            </div>
            <p className="GCHtext-xs text-white">0°以上为露点</p>
        </div>
    )
}
...
const WeatherUV = (props) => {
    ...
    if(0 < UV && UV <= 2){UVText = '紫外线最弱'}
    if(2 < UV && UV <= 4){UVText = '紫外线较弱'}
    if(4 < UV && UV <= 6){UVText = '紫外线中等'}
    if(6 < UV && UV <= 8){UVText = '紫外线强'}
    if(8 < UV && UV <= 10){UVText = '紫外线强烈'}
    if(10 < UV){UVText = '紫外线极强'}

    return(
        <div className={`w-full h-full z-20 px-4 py-3 flex flex-col justify-between rounded-2xl ${WeatherCard({cloud:cloud})}`}>
            <div className="w-full flex flex-col gap-2">
                <div className="flex flex-row gap-2 items-center">
                    <IconUv w = '18' h = '18' fill = 'rgba(255,255,255,0.6)'/>
                    <p className="GCHtext-xs text-white/60">紫外线指数</p>
                </div>
                <div className="w-full flex flex-col gap-3">
                    <p className="GCHtitle-base text-lg">{UV}级</p>
                    <div className="relative w-full h-1 flex items-center  rounded-full bg-gradient-to-r from-[#facc15] via-[#f97316] to-[#a855f7]">
                         <div className={`absolute left-[${left}] w-2 h-2 rounded-full bg-white shadow-md `}></div>
                    </div>
                </div>
                
            </div>
            <p className="GCHtext-xs text-white">{UVText}</p>
        </div>
    )
}
...
const WeatherBgColor = (props) => {
    ...
    return(
        <div className={`absolute  top-0 left-0 w-[67.5rem] h-[24rem] rounded-2xl ${WeatherBg({cloud:cloud})}`}></div>
    )
}
